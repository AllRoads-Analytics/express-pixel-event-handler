...
var _followValues$uid, _followValues$sid, _followValues$params$;
...
var Helper = /*#__PURE__*/function () {
  ...
  _createClass(Helper, null, [{
    ...
    key: "addMinutesToDate",
    value: function addMinutesToDate(Date, minutes) {
    ...
    value: function addFollowParam(a) {
      if (Config.follow) {
        try {
          ...
          linkUrl.searchParams.set(pixelFuncName + '_follow', uid + '.' + sid + '.' + btoa(JSON.stringify({
            utms: Cookie.getUtms()
          })) + '.' + new Date().getTime());
          a.href = linkUrl.href;
        } catch (e) {
...
var Follow = /*#__PURE__*/function () {
  ...
  _createClass(Follow, null, [{
    key: "getValuesFromUrl",
    value: function getValuesFromUrl() {
      ...
      try {
        if (Helper.isPresent(uid_param)) {
          ...
          if (parts.length === 4) {
            var now = new Date();
            var paramDate = new Date(parseInt(parts[3]));
            paramDate.setTime(paramDate.getTime() + 10 * 1000); // add 10 seconds

            var follow_uid = parts[0];
            var follow_sid = parts[1];
            var params = JSON.parse(atob(parts[2]));

            if (!isNaN(paramDate.getTime()) && now.getTime() < paramDate.getTime() && 'string' === typeof follow_uid && follow_uid.length < 30 && 'string' === typeof follow_sid && follow_sid.length < 30 && 'object' === _typeof(params)) {
              return {
                uid: follow_uid,
                sid: follow_sid,
                params: params
            ...
            console.error('Invalid allroads_follow param (length)');
          }
        }
      } catch (e) {
...
var followValues = Follow.getValuesFromUrl(); // Get UID from A. URL; B. Cookie; C. New.

var uid = (_followValues$uid = followValues.uid) !== null && _followValues$uid !== void 0 ? _followValues$uid : Helper.guid();
var sid = (_followValues$sid = followValues.sid) !== null && _followValues$sid !== void 0 ? _followValues$sid : Helper.sid(); // Set UID & SID cookies.

Cookie.set('uid', uid, 2 * 365 * 24 * 60); // 2 years exp

Cookie.set('sid', sid); // session exp
// Get UTMs from "follow" param.

var utms = 'undefined' !== typeof followValues.params ? (_followValues$params$ = followValues.params.utms) !== null && _followValues$params$ !== void 0 ? _followValues$params$ : {} : {};

if (!Object.keys(utms).length) {
  // Get UTMs "regular" way, from URL.
  utms = Url.getUtms();
} // save any utms through as session cookies


if (Object.keys(utms).length) {
  Cookie.set('utm', JSON.stringify(utms));
